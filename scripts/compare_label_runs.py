"""
compare_label_runs.py — diff two label runs, field by field.
...
backup JSONL against the current label log over the photo_ids they share. Paths are
relative to include/data/labels.

Usage:
    python scripts/compare_label_runs.py
    python scripts/compare_label_runs.py --old archive/raw_labels.v2.bak.jsonl --examples 8
"""
...
DATA = Path(__file__).resolve().parents[1] / "include" / "data" / "labels"
...
def main() -> None:
    ...
    ap.add_argument("--old", default="archive/raw_labels.v2.bak.jsonl")
