"""
A local HTML sheet for hand-checking labels.
...
Nothing is uploaded; thumbnails are generated locally. Mark anything wrong while
scrolling, then use "Copy corrections" at the bottom.
"""
...
from include.jwst_pipeline.assemble import KEPT
...
def stratified(rows: list[dict], n: int, seed: int = 42) -> list[dict]:
...
def _thumbnail(photo_id: str, thumbs: Path) -> str | None:
    from PIL import Image

    dest = thumbs / f"{photo_id}.jpg"
    if dest.exists():
        return dest.name
    # The 1024px dataset copy is much quicker to shrink than the original.
    for src in (paths().dataset_images / f"{photo_id}.jpg", paths().images / f"{photo_id}.jpg"):
        if src.exists():
            try:
                img = images.open_rgb(src)
                img.thumbnail((THUMB_PX, THUMB_PX), Image.LANCZOS)
                img.save(dest, format="JPEG", quality=82)
                return dest.name
            except Exception as exc:
                log.warning("thumbnail failed for %s: %s", photo_id, exc)
    return None
...
def build_sheet(n: int = 100) -> Path:
    src = paths().dataset / f"{KEPT}.parquet"
    cols = ["photo_id", "title", "description", "subject", "modality", "object_name",
            "object_name_verified", "instrument", "rationale", "source_tag_label", "flickr_url"]
    con = duckdb.connect()
    try:
        rows = [dict(zip(cols, r)) for r in con.execute(f"SELECT {', '.join(cols)} FROM '{src}'").fetchall()]
    finally:
        con.close()

    sample = stratified(rows, n)
    out_dir = paths().review
...
<div class="card" id="c-{pid}">
  ...
  <div class="lab">
    ...
    {old_html}
    <div class="rat">{html.escape(r['rationale'] or '')}</div>
    <div class="cap"><b>{html.escape(r['title'] or '')}</b><br>{html.escape((r['description'] or '')[:420])}</div>
...
<script>{JS}</script>"""
    ...
    out.write_text(page)
    log.info("wrote review sheet of %d rows to %s", len(sample), out)
    return out
